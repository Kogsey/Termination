...
namespace Termination
{
    public static class TerminationUtils
    {
        ...
        public static Vector2 RegularToComponantVector(Vector2 origin, float degrees, float magnitude, Vector2 postoffset)
        {
            Vector2 returnvector;
            returnvector.X = origin.X + ((float)Math.Cos(MathHelper.ToRadians(degrees)) * magnitude) + postoffset.X;
            returnvector.Y = origin.Y + ((float)Math.Sin(MathHelper.ToRadians(degrees)) * magnitude) + postoffset.Y;
        ...
        {
            ErrorLogger.Log(String.Format("[Engineer][{0}] {1}", DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"), message));
        }

        [Obsolete]
        public static void Log(string format, params object[] args)
        {
            ErrorLogger.Log(String.Format("[Engineer][{0}] {1}", DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"), String.Format(format, args)));
        }
