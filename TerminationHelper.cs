...
namespace Termination
{
    public static class TerminationHelper
    {
        ...
        public static Vector2 PolarPos(Vector2 Point, float Distance, float Angle, int XOffset = 0, int YOffset = 0)
        {
            ...
            ReturnedValue.X = (Distance * (float)Math.Sin((double)TerminationHelper.RadtoGrad(Angle)) + Point.X) + XOffset;
            ReturnedValue.Y = (Distance * (float)Math.Cos((double)TerminationHelper.RadtoGrad(Angle)) + Point.Y) + YOffset;
