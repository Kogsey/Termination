...
{
	public class ElectronicEyePrismBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / ElectronicEyePrismGun.NumBeams;
		...
		public override void AI()
		{
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / ElectronicEyePrismGun.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > ElectronicEyePrismGun.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - ElectronicEyePrismGun.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * ElectronicEyePrismGun.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / ElectronicEyePrismGun.NumBeams) % BeamHueVariance + BeamColorHue;
